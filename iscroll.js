...
function iScroll (el, options) {
	...
	that.wrapper._scrollerX = 0;
	that.wrapper._scrollerY = 0;
...
iScroll.prototype = {
	...
	setPosition: function (x, y, hideScrollBars) {
		...
		that.wrapper._scrollerX = that.x;	// Used by the drag-n-drop
		that.wrapper._scrollerY = that.y;
	...
	touchMove: function(e) {
		var that = this,
			...
			leftDelta = pageX - that.touchStartX,
			topDelta = pageY - that.touchStartY,
			newX = 0,
...
/*		if (newX >= 0 || newX < that.maxScrollX) {
		...
		if (that.dist > 6) {			// 5 pixels threshold is needed on Android, but also on iPhone looks more natural
			...
			that.dist+= that.distX + that.distY;
		}
